import { ComponentRef, NgZone } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { NavController } from '../../providers/nav-controller';
import { RouteView } from './stack-utils';
export declare class StackController {
    private containerEl;
    private router;
    private navCtrl;
    private zone;
    private viewsSnapshot;
    private views;
    private runningTransition?;
    private skipTransition;
    private tabsPrefix;
    private activeView;
    private nextId;
    constructor(tabsPrefix: string | undefined, containerEl: HTMLIonRouterOutletElement, router: Router, navCtrl: NavController, zone: NgZone);
    createView(enteringRef: ComponentRef<any>, activatedRoute: ActivatedRoute): RouteView;
    getExistingView(activatedRoute: ActivatedRoute): RouteView | undefined;
    setActive(enteringView: RouteView): Promise<void>;
    canGoBack(deep: number, stackId?: string | undefined): boolean;
    pop(deep: number, stackId?: string | undefined): void;
    startBackTransition(stackId?: string | undefined): void;
    endBackTransition(shouldComplete: boolean): void;
    getLastUrl(stackId?: string): RouteView | undefined;
    private getActiveStackId;
    private getStack;
    private insertView;
    private cleanup;
    private transition;
}
